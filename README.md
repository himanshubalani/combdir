...
A PowerShell script that combines all files in a directory into a single text file for easy upload to AI services like Google AI Studio, Claude, or ChatGPT.
...
| `-SkipFolders <names>` | Comma-separated folder names to always skip, matched anywhere in the path. Example: `"ui,icons,generated"` |
...
- Any previously generated output files matching `combined_*.txt`
...
Each file in the combined output is wrapped with clearly marked delimiters:
...
4. Reads each remaining file and writes it to the output with start/end markers
5. Appends a summary with total files processed and output size
